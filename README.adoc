...
setup()::
  ...
  You may use it to check for required variables (using `required_var()`), to define some common variables or anything else that should be run before `pre-backup()` and `post-backup()`.
